    ...
    link : (scope, element, attr) ->
        ...
        $rootScope.$on "user:updated", (ev, user)=>
            ...
            if      scope.nodes_max    > -1 and scope.max_nodes_count >= scope.nodes_max - 10
                ...
                scope.templateUrl = "/partial/components/plan-overfill-message/attention-nodes.html"
                ...
                scope.templateUrl = "/partial/components/plan-overfill-message/warning-nodes.html"
            else if scope.topics_max   > -1 and scope.topics_count    >= scope.topics_max
                # Attention: topics count == max
                scope.templateUrl = "/partial/components/plan-overfill-message/attention-topics.html"
                element.addClass("plan-overfill-message--attention")
            else
                scope.hidden = true
]
