...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for topic in topics:
            try:
                group = orm['Auth.Group'].objects.get(name="{id}_administrator".format(id=topic.ontology_as_mod))
            except orm['Auth.Group'].DoesNotExist:
                new_group = orm['Auth.Group'](name="{id}_administrator".format(id=topic.ontology_as_mod))
                new_group.save()
                if topic.author is not None:
                    topic.author.groups.add(new_group)
                    topic.save()

    def backwards(self, orm):
        groups = orm['Auth.Group'].objects.all()

        for group in groups:
            if re.search('_administrator$', group.name):
                group.delete()
