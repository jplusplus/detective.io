...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for topic in topics:
            group_name = "{0}_administrator".format(topic.ontology_as_mod)
            try:

                group = orm['auth.Group'].objects.get(name=group_name)
                topic.administrator_group = group
                topic.save()
            except orm['auth.Group'].DoesNotExist:
                print "Could not find group {0}".format(group_name)
                pass
