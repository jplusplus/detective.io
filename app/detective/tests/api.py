...
class ApiTestCase(ResourceTestCase):
    ...
    def test_forms_summary(self):
        resp = self.api_client.get('/api/common/v1/summary/forms/?topic=energy', format='json', authentication=self.get_super_credentials())
        self.assertValidJSONResponse(resp)
        # Parse data to check the number of result
        data = json.loads(resp.content)
    ...
    def test_types_summary(self):
        resp = self.api_client.get('/api/common/v1/summary/types/?topic=energy', format='json', authentication=self.get_super_credentials())
        self.assertValidJSONResponse(resp)
