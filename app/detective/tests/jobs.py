...
class JobsTestCase(ResourceTestCase):
    ...
    def test_bulk_upload(self):
        ...
        files = (
            (
                "pillen.csv", (
                    ("Pill_id,name,logo,image,image_back,break_notch,color,weight,diameter,thickness,place,place__sources__,date,more_infos,molecules_contained__sources__"),
                    ("57713,Peer,,,,True,yellow,,,,,source de peer,,,picsou mag"),
                    ("57457,Apple,,,,False,,,,,,sources de apple||une autre source,,,"),
                    ("57229,newone,,,,False,,,,,,,,,"),
                    ("57217,Ahorn,,,,False,,,,,,,,,"),
        ...
        # test sources
        def get_pilule(pilule_id):
            resp = self.api_client.get('/api/detective/test-pillen/v1/pill/%d/' % (pilule_id))
            resp = json.loads(resp.content)
            return resp
        sources      = get_pilule(Pill.objects.get(name="Peer").id)["field_sources"]
        place_source = next((d for d in sources if d.get("field") == "place"), None)
        self.assertIsNotNone(place_source)
        self.assertEquals(len(sources), 2)
        self.assertEquals(place_source.get("reference"), "source de peer")
        sources      = get_pilule(Pill.objects.get(name="Apple").id)["field_sources"]
        place_source = [d for d in sources if d.get("field") == "place"]
        self.assertEquals(len(place_source), 2)
