...
class JobsTestCase(ResourceTestCase):
    ...
    def test_bulk_upload(self):
        ...
        files = (
            (
                "pillen.csv", (
                    ("Pill_id,name,logo,image,image_back,break_notch,color,weight,diameter,thickness,place,place__sources__,date,more_infos"),
                    ("57713,Peer,,,,True,yellow,,,,,source de peer,,"),
        ...
        def get_pilule(pilule_id):
            ...
            resp = json.loads(resp.content)
            return resp
        peer = Pill.objects.get(name="Peer")
        sources = get_pilule(peer.id)["field_sources"]
        place_source = next((d for d in sources if d.get("field") == "place"))
        self.assertIsNotNone(place_source)
        self.assertEquals(place_source.get("reference"), "source de peer")
        apple = Pill.objects.get(name="Apple")
        sources = get_pilule(apple.id)["field_sources"]
